...
export async function onRequestGet({env}) {
  try {
    ...
    if(!row){const updated=nowIso();await env.DB.prepare('INSERT INTO app_state(id,data,updated_at) VALUES(1,?,?)').bind(JSON.stringify(emptyState),updated).run();return json({state:emptyState,updatedAt:updated});}
    let state; try{state=JSON.parse(row.data)}catch{state=emptyState}
    return json({state,updatedAt:row.updated_at});
  } catch(e) { return json({error:'No se pudo leer la base de datos.',detail:detail(e)},500); }
}
...
export async function onRequestPost({request,env}) {
  ...
  } catch(e) { return json({error:'No se pudo guardar en D1.',detail:detail(e)},500); }
}
